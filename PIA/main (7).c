...
int create() //1st option in the menu
{
    int r=0;
    char name[NAME + 1];
    ...
    while((c = fgetc(guests)) != EOF)
    {
        if (c == '\n')
        {
            count++; //count for number of rooms occupied
        }
    }
    rewind(guests);

   if (count<ROOM) // if there is vacancy
   {
       srand(time(NULL));
       FILE *guests = fopen("guests.txt", "a");
       printf("Enter name. Example: Luis\n");
       fflush (stdin);
       scanf("%s", name);
...
int disp(int x) //function to avoid repetition of room number
{
    char c;
...
int pay() //2nd option in the menu
{
    char desname[NAME];
    ...
    if( (strcmp (desname, xame) == 0) && (strcmp (despassword, xassword) == 0)) //check if username and password coincide
    {
        ...
        break;
    } else          //if username and password don't coincide
    {
        m = 1;
    }
    }
